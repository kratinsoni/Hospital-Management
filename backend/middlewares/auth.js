...
export const isAdminAuthenticated = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!token) {
    return next(new ErrorHandler("Admin Not Authenticated!", 400)); //The function checks if the adminToken cookie is present and, if not, throws an error.
  }
...
export const isAdminOrDoctorAuthenticated = catchAsyncErrors(
  async (req, res, next) => {
    ...
    if (!token) {
      return next(new ErrorHandler("Admin or Doctor Not Authenticated!", 400));
    }
    const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY);
    req.user = await User.findById(decoded.id);
